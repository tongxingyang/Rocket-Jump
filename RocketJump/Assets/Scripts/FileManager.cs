...
public class FileManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        void LoadDocument()
        {
            ...
            else if (Application.platform == RuntimePlatform.WindowsPlayer)
            {
                if (!File.Exists(Application.persistentDataPath + "/GameData.xml"))
                {
                    TextAsset gameData = (TextAsset)Resources.Load("GameData");
                    doc.LoadXml(gameData.text);
                    SaveData("fileVersion", fileVersion);
                }
                else
                {
                    doc.Load(Application.persistentDataPath + "/GameData.xml");
    ...
    public static void SaveData(string path, string value)
    {
        doc.SelectSingleNode("//game/" + path).InnerText = value;
